...
class GoogleGeocoding
{
    ...
    public function get()
    {
        ...
        if (!isset($body['status']) || ($body['status'] !== 'OK' && $body['status'] !== 'ZERO_RESULTS')) {
            if (config('google-geocoding.log_errors')) {
                if ($body['error_message']) {
                    Log::error(
                        'Google returned an error when requesting geocoding results with url [' . $this->getUrl(
                        ) . ']: ' . $body['error_message']
                    );
                } else {
                    Log::error(
                        'Google returned an error when requesting geocoding results with url [' . $this->getUrl(
                        ) . ']: ' . json_encode($body['error_message'])
                    );
                }
            }
        ...
        return cache()->remember(
            $this->getCacheKey(),
            now()->addMinutes(config('google-geocoding.cache_duration')),
            function () use ($body) {
                $this->storeInLogs($body);
